...
vector < string > 
GetDataTransport ( std::istream &input )
{
  string line ;
  getline ( input,line );
  // данные
  int data_line = stod ( line );
...
vector < string > 
GetUserAll ( std::istream &input )
{
  // users ввод
  string line;
  getline ( input,line );
  int users_line = stod ( line );
...
Stop ParsingStop ( const std::string &line )
{
    ...
    if (distance.find(' ')!=string::npos)
    {
        while ( distance.size( ) > 0 )
        {
            ...
            /* cout<< "stops: " ; */
            size_t distance_to_stop = stoi ( distance.substr( 0, distance.find( 'm' )) );
            distance.erase( 0, distance.find( "to " ) + 3 );
            string name = distance.substr( 0, distance.find( ',' ));
            /* cout<< distance_to_stop << '|' << name << '|' <<endl; */
...
Bus ParsingBus ( const std::string& line )
{
    string index = line.substr( 0, line.find( ' ' ));
    if ( index != "Bus" )
    {
    ...
    int stops_count = count ( line.begin( ), line.end( ), '>' );
    if ( stops_count > 0 )
    {
        string stops = line;
        bus.stops.reserve( ++stops_count );
    ...
    int stops_count_reverse = count ( line.begin( ), line.end( ), '-' ) * 2 ;
    if ( stops_count_reverse++ > 0 )
    {
           bus.stops.reserve( stops_count_reverse );
