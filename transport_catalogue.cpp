...
TransportCatalogue ( std::istream& input )
{
    ...
    for ( string line : data_catalog )
    {
        ...
        if ( index == "Stop" ){ stops.push_back( Detail::ParsingStop ( line ));
                               stops_map [stops.back( ).name_stop] = & stops.back( );
                              }
        if ( index == "Bus" ){ buses.push_back( Detail::ParsingBus ( line ));  
...
double TransportCatalogue::
LengthBusStops ( const string& bus )
{
    double result = 0.0;
...
bool TransportCatalogue::
StatusUserBus ( const Detail::Bus& bus )
{
...
bool TransportCatalogue::
StatusUserStop ( const Detail::Stop& stop )
{
    return find_if ( data_user_all.begin( ), data_user_all.end( ),
...
Detail::Stop  
TransportCatalogue::
GetStop ( const std::string& str ) const
{
   if ( stops_map.find( str )!= stops_map.end( ))
   {
       return * stops_map.at( str );
   } 
   return {};
}
...
Detail::Bus  
TransportCatalogue::
GetBuses ( const std::string& str ) const 
{
    try
    {
        return * bus_map.at( str ); 
    }
    catch ( ... )
    {
...
/* InitBusesOnStop ( ) */
/* { */
/*     // 1. проходимся по всем остановкам */
/* for ( auto stop: stops ) */
/*     { */
/*     // 2. Проходимся по всем автобусам */
/* for ( auto bus: buses ) */
/*         { */
/*     // 3. Есть ли у автобуса упоминание об этой остановке? */
/* if ( find_if ( bus.stops.begin( ), bus.stops.end( ), */
/*                  [stop]( const string& stops ){ */
/* return ( stop.name_stop == stops ); */
/*                  } )!= bus.stops.end( ) */
/* )// if */
/*             { */
/* buses_on_stop[stop.name_stop].insert( bus.name_bus ); */
/*             } */
/*         } // for buses */
/*     } // for stops */
/* } */
...
TransportCatalogue::
BusesOnStop ( const std::string_view &name )
{
...
void 
TransportCatalogue::
AddPairStop ( const std::string_view name_stop )
{
...
ShowDistance ( )
{
    ...
    {
        cout<< i.first.first-> name_stop << ' ' 
 << i.first.second-> name_stop << " = " 
 << i.second <<endl;
    }
}
...
/* size_t */ 
/* TransportCatalogue:: */
/* RouteBusDistance ( const Bus& bus ) */
/* { */
/* size_t totalDistance = 0; */
/*     // Проходим по всем парам остановок */
/* for ( const auto& pair : distance_ ){ */
/*         // Получаем указатели на остановки */
/* const Stop * stop1 = pair.first.first; */
/* const Stop * stop2 = pair.first.second; */
/* size_t distance = pair.second; */


/*         // Проверяем, что остановки принадлежат текущему автобусу */
/* if ( bus.stops_link.count( stop1-> name_stop ) */
/*                 && bus.stops_link.count( stop2-> name_stop )) */ 
/*         { */
/* cout<< stop1-> name_stop << ' ' << stop2-> name_stop << " = " << distance <<endl;; */
...
size_t 
TransportCatalogue::
RouteBusDistance ( const Detail::Bus& bus )
{
    size_t totalDistance = 0;
    if (bus.stops.size()<=0)
    {
